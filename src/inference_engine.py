"""
Inference Rules Engine for Restaurant Recommendation System

Applies inference rules to determine additional restaurant properties based on initial properties.
"""

class InferenceEngine:
    """
    Applies inference rules to determine additional restaurant properties.
    """
        ...
        self.inference_rules = [
            {
                ...
                'antecedent_type': 'AND',  # Both conditions must be true
    ...
    def evaluate_antecedent(self, rule, restaurant):
        ...
        Args:
            rule (dict): The inference rule
            restaurant (dict): Restaurant data with properties
            
        Returns:
            bool: True if antecedent conditions are satisfied
        """
    ...
    def _check_single_condition(self, condition, restaurant):
        ...
        """
        # Map conditions to restaurant properties
        if condition == 'cheap':
    ...
    def apply_rules(self, restaurant):
        ...
        Returns:
            dict: Dictionary containing inferred properties and reasoning
        """
        inferred_properties = {}
        applied_rules = []
        reasoning = []
                ...
                if consequent in inferred_properties:
                    if inferred_properties[consequent]['value'] != consequent_value:
                        ...
                        if consequent == 'romantic' and {rule['id'], old_rule_id} == {5, 6}:
                            # This is the busy vs long_stay conflict for romantic property
                            conflict_info = {
                                'property': consequent,
                            ...
                            # For other conflicts, keep existing behavior
        ...
        # Update possible_properties with inferred values
    ...
    def filter_restaurants_by_requirements(self, restaurants, user_requirements):
        ...
        Args:
            restaurants (list): List of candidate restaurants
            user_requirements (dict): Additional requirements like {'touristic': True, 'romantic': True, etc.}
            
        Returns:
            dict: Contains 'restaurants' list and optional 'conflict' info for user resolution
        """
        ...
        for restaurant in restaurants:
            ...
            if ('conflict' in inference_result and 
                ...
                user_requirements['romantic'] is True):
                # Store restaurants with romantic conflicts for user resolution
            ...
            for req_property, req_value in user_requirements.items():
                if req_value is not None:  # Only check properties user cares about
            ...
            if meets_requirements:
                # Add inference results to restaurant data for later use
                restaurant_with_inference = restaurant.copy()
                restaurant_with_inference['inference_result'] = inference_result
        ...
        if romantic_conflicts and 'romantic' in user_requirements and user_requirements['romantic'] is True:
            return {
                'restaurants': filtered_restaurants,
                'has_romantic_conflicts': True,
                'conflict_restaurants': romantic_conflicts
            }
        
        return {
            'restaurants': filtered_restaurants,
            'has_romantic_conflicts': False,
            'conflict_restaurants': []
        }
    ...
    def explain_recommendation(self, restaurant):
        ...
        Args:
            restaurant (dict): Restaurant with inference_result data
            
        Returns:
            str: Human-readable explanation with inference reasoning
        """
        ...
        if inferred_properties.get('romantic') is not None:
            ...
            if romantic_value:
                # Find the reasoning for romantic = True
                ...
                # Find reasoning for romantic = False  
                for reason in reasoning:
        ...
        # Add other property explanations
        ...
        # Handle contradictions explanation
...
def test_inference_engine():
    """
    Test the inference engine with sample restaurant data.
    """
    engine = InferenceEngine()
    
    # Test restaurant 1: cheap + good food
    restaurant1 = {
    ...
    result1 = engine.apply_rules(restaurant1)
    print("Restaurant 1 (cheap + good food):")
    print(f"Inferred properties: {result1['inferred_properties']}")
    print(f"Applied rules: {result1['applied_rules']}")
    print(f"Reasoning: {result1['reasoning']}")
    print()
    
    # Test restaurant 2: Romanian + busy + long stay (contradiction case)
    restaurant2 = {
    ...
    result2 = engine.apply_rules(restaurant2)
    print("Restaurant 2 (Romanian + busy + long stay):")
    print(f"Inferred properties: {result2['inferred_properties']}")
    print(f"Applied rules: {result2['applied_rules']}")
    print(f"Reasoning: {result2['reasoning']}")
    print(f"Has contradictions: {result2['has_contradictions']}")
    print()
